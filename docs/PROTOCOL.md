...
a1 = 0x0100
a2 = device selector
a3 = packed 32-bit colour/brightness value
```
...
|---|---|
| `0` | all LED devices |
| `6` | all keyboard LED devices |
| `n` | single device index |

The stock UI exposes three keyboard zones (A/B/C); the persisted settings carry
five packed values per profile, so the additional devices are believed to be the
light bar and a further zone. Zone-index-to-device mapping is **not yet
confirmed** and should be probed by reading, not guessing.

Packed value (`a3`) layout, inferred from stock profile data:
...
→ low 24 bits are RGB, high byte carries mode/brightness. Effects seen in the
stock UI: Static, Breathing, Colorful cycle, Ambilight. The encoding of effect
and brightness within the high byte is **not yet confirmed**.
